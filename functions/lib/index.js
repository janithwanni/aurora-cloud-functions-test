...
exports.initQuestion = functions.database
    ...
    if (snapshot.after.val() == true) {
        ...
        let teamID = (context.params["leaderboardRowID"] + "").split("-")[2];
...
exports.onQuestionTimeout = functions.database
    ...
    let score = 0;
    let time;
    let timeLeft; //get from the general timeleft node
    let ringLevel;
    let totalDistance;
    let timeExceeded = 1; //check this from the currentTime field in the progress
    /* //teamID from params
...
exports.onAnswerGiven = functions.database
    .ref("teamCurrentQuestion/{teamID}/timeExceeded")
    .onUpdate((snapshot, context) => { });
